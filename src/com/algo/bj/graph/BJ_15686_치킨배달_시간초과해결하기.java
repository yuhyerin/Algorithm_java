...
public class BJ_15686_치킨배달_시간초과해결하기 {
	static int N,M;
	...
	public static void main(String[] args) throws Exception {
		...
		pickChicken(0, 0, pickChickens); // 치킨집 M개 고르기.
		System.out.println(min);
		
	}

	private static void pickChicken(int start, int cnt, ArrayList<int[]> pickChickens) {
		...
		for(int i = start; i < chickens.size(); i++) {
			...
			if(pick[cy][cx]==0) { // 아직 안뽑은 치킨집중에 
				pick[cy][cx]=2; // 뽑은 치킨집 표시
				pickChickens.add(new int[] {cy,cx});
				pickChicken(i+1, cnt+1, pickChickens);
				pick[cy][cx]=0; // 뽑은거 원복.
